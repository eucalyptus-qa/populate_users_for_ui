...
###	CREATE FOR TESTING UI		090512
sub add_account_user_ui_login_key{
	my $account = shift @_;
	my $user = shift @_;
	my $password = shift @_;

	print "\n";
	print "########################### EUARE-USERADDLOGINPROFILE ##############################\n";

	### add login profile
	print "$ENV{'QA_CLC_IP'} :: euare-useraddloginprofile --delegate $account -u $user -p $password\n";
#	system("ssh -o ServerAliveInterval=1 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no root\@$ENV{'QA_CLC_IP'} \"source /root/cred_depot/$account/admin/eucarc; euare-useraddloginprofile --delegate $account -u $user -p $password\" ");
	system("ssh -o ServerAliveInterval=1 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no root\@$ENV{'QA_CLC_IP'} \"source /root/cred_depot/eucalyptus/admin/eucarc; euare-useraddloginprofile --delegate $account -u $user -p $password\" ");
	sleep(3);

	print "\n";
	print "\n";
	print "########################### EUARE-USERADDKEY ##############################\n";

	### add key
	print "$ENV{'QA_CLC_IP'} :: euare-useraddkey --delegate $account -u $user\n";
#	system("ssh -o ServerAliveInterval=1 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no root\@$ENV{'QA_CLC_IP'} \"source /root/cred_depot/$account/admin/eucarc; euare-useraddkey --delegate $account -u $user");
	system("ssh -o ServerAliveInterval=1 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no root\@$ENV{'QA_CLC_IP'} \"source /root/cred_depot/eucalyptus/admin/eucarc; euare-useraddkey --delegate $account -u $user\" ");
	print "\n";
	print "\n";
	
	print "[TEST_REPORT]\tSucceeded in creating account user password [$account, $user, $password]!\n\n";
