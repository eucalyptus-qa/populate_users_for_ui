...
if ( @ARGV > 0 ){
	$given_user_password = shift @ARGV;
};
...
my $user_password = "default-qa-password";
...
if( $given_user_password ne "" ){
	$user_password = $given_user_password;
}elsif( is_test_account_user_password_from_memo() ){
	$user_password = $ENV{'QA_MEMO_TEST_ACCOUNT_USER_PASSWORD'};
};

print "\n";
print "TEST ACCOUNT NAME [$account_name]\n";
print "TEST USER NAME [$user_name]\n";
print "TEST USER PASSWORD [$user_password]\n";
print "\n";
...
###
### add user UI login key
###

add_account_user_ui_login_key($account_name, $user_name, $user_password);
print "\n";
...
print "\n";
print "[TEST_REPORT]\tCREATE ACCOUNT AND USER FOR UI HAS BEEN COMPLETED\n";
print "\n";
